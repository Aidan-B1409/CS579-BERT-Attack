...
def dump_features(features, output):
    ...
    df = pd.DataFrame().from_dict({'text': [feature.final_adverse for feature in features], 'label': [feature.label for feature in features]})
    df.to_csv(f'{output_json}.csv', index=False)
